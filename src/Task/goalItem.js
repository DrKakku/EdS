...
export default function GoalItem(props) {
  ...
  const progress = () => {
    if (
      props.startDate > new Date(1991, 0, 0, 0, 0) ||
      props.endDate > new Date(1992, 0, 0, 0, 0)
    ) {
      return (
        <View>
          {DateDynamic()}
          <View>
            ...
            <View>{TimesDynamic()}</View>

            <Progress.Bar
              color={colorGradientRender(percent)}
  ...
  const DateDynamic = () => {
    if (props.startDate.getDay() == props.endDate.getDay()) {
      ...
      props.endDate.getDate() - props.startDate.getDate() <= 6
    ) {
      return (
        <View style={styles.title}>
          <View style={{ flex: 1 }}>
            <Text style={{ color: colorArr[0] }}>
              {" "}
              {dayWord(props.startDate.getDay())} {props.startDate.getHours()}:
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ color: colorArr[9] }}>
              {" "}
              {dayWord(props.endDate.getDay())} {props.endDate.getHours()}:
              {props.endDate.getMinutes()}{" "}
            </Text>
          </View>
        </View>
      );
        ...
        return (
          <View style={props.BGstyle.title}>
            <View style={{ flex: 1 }}>
              <Text style={{ color: colorArr[0] }}>
                {" "}
                {props.startDate.getDate()} {dayWord(props.startDate.getDay())}{" "}
                {props.startDate.getHours()}:{props.startDate.getMinutes()}{" "}
              </Text>
            </View>
            ...
            <View style={{ flex: 1 }}>
              <Text style={{ color: colorArr[9] }}>
                {" "}
                {props.endDate.getDate()} {dayWord(props.endDate.getDay())}{" "}
                {props.endDate.getHours()}:{props.endDate.getMinutes()}{" "}
              </Text>
            </View>
          </View>
  ...
  const TimesDynamic = () => {
    ...
    if (TimePercent() == 1) {
      ...
      return <Text style={{ color: color, fontWeight: "bold" }}>Tic toc</Text>;
    }
  };

  const colorGradientRender = (
    per,
    colAr = [
      "#277da1",
      "#577590",
    ...
    <TouchableOpacity activeOpacity={0.4}>
      <View style={props.BGstyle.task}>
        <View style={{ flex: 1 }}>
          <Text
            style={{
              color: props.BGstyle.textcol.color,
              fontSize: 20,
              marginBottom: 10,
            }}
          >
        ...
        <View style={styles.title2}>
          <View style={{ alignSelf: "center", marginTop: 15 }}>
            <Text style={{ color: props.BGstyle.textcol.color, fontSize: 10 }}>
              {" "}
              {props.desc}
            </Text>
          </View>
