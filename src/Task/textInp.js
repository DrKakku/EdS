...
const InputTask = (props) => {
  ...
  const TimeRender = () => {
    if (
      StartDate <= new Date(1991, 0, 0, 0, 0, 0) ||
      EndDate <= new Date(1992, 0, 0, 0, 0, 0)
    ) {
    } else {
      return (
        <View style={props.bgStyle.title}>
          <View style={{ flex: 1 }}>
            <Text>
              Start:-{Task.startDate.getDate()}{" "}
              {dayWord(Task.startDate.getDay())} {Task.startDate.getHours()}:
              {Task.startDate.getMinutes()}.{Task.startDate.getSeconds()}
            </Text>
          </View>

          <View style={{ flex: 1 }}>
            <Text>
              End:-{Task.endDate.getDate()} {dayWord(Task.endDate.getDay())}{" "}
              {Task.endDate.getHours()}:{Task.endDate.getMinutes()}.
              {Task.endDate.getSeconds()}
            </Text>
          </View>
        </View>
      );
      ...
      >
        ...
        >
          <View style={Style.body}>
            <View>
              <Text style={props.bgStyle.textcol}>
                So <Text style={props.bgStyle.themeCol}>What</Text> Is it
              </Text>
              ...
              <View style={Style.Date}>
                <View style={Style.buttonContanier}>
                  <Button
                    color={props.bgStyle.themeCol.color}
                    style={Style.button}
                ...
                <DateTimePickerModal
                  isDarkModeEnabled={true}
                  mode="datetime"
                  isVisible={enddatestate}
                  onConfirm={handleendConfirm}
                  onCancel={hideendDatePicker}
                />
              </View>

              <View>{TimeRender()}</View>
